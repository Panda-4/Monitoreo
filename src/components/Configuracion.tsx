...
import { Save, UserCog, Database, Shield, Bell, Plus, X, Edit2, UserX, Settings, CheckCircle, Send } from 'lucide-react';
...
export default function Configuracion() {
  ...
  const handleEnviarNotificacion = async () => {
    ...
    try {
      const res = await authFetch(`${API_BASE}/api/notificaciones/enviar-todos`, {
        method: 'POST',
        body: JSON.stringify({ mensaje: mensajeNotificacion.trim() }),
      });
      if (res.ok) {
        setMensajeNotificacion('');
        setSuccessMsg('Notificación enviada exitosamente a todos los usuarios');
        setTimeout(() => setSuccessMsg(''), 3000);
      } else {
        const err = await res.json().catch(() => ({}));
        setErrorMsg(err.error || 'Error al enviar notificación');
        setTimeout(() => setErrorMsg(''), 3000);
      }
    } catch (error) {
      setErrorMsg('Error de conexión al enviar notificación');
      setTimeout(() => setErrorMsg(''), 3000);
    } finally {
  ...
  const rolLabel = (rol: string) => {
    ...
    return <span className={`${colors[rol] || 'bg-gray-100 text-gray-800 dark:bg-slate-700 dark:text-slate-300'} px-2.5 py-1 rounded-lg text-xs font-bold`}>{rol}</span>;
  };
  ...
  return (
    <div className="w-full max-w-5xl mx-auto py-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {/* Success Toast */}
      {successMsg && (
        <div className="mb-6 flex items-center gap-2 p-3 bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800 rounded-xl text-green-700 dark:text-green-300 text-sm font-medium animate-in slide-in-from-top-2">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1 bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl border border-gray-200 dark:border-slate-700 rounded-2xl shadow-sm p-8 min-h-[500px]">
          ...
          {activeTab === 'notificaciones' && (
            <div className="space-y-6 animate-in fade-in">
              <div className="border-b border-gray-100 dark:border-slate-700 pb-4">
                <h3 className="text-xl font-bold text-gray-800 dark:text-slate-100">Enviar Notificación</h3>
                <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">Envía un mensaje a todos los usuarios del sistema.</p>
              </div>

              <div className="bg-gray-50/50 dark:bg-slate-900/50 border border-gray-200 dark:border-slate-700 rounded-2xl p-6 space-y-4">
                <div className="space-y-1.5">
                  <label className="block text-sm font-semibold text-gray-700 dark:text-slate-300">Mensaje</label>
                  <textarea
                    ...
                    className="w-full bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-600 text-gray-800 dark:text-slate-100 text-sm rounded-xl p-4 focus:ring-2 focus:ring-gem-primary/20 focus:border-gem-primary transition-colors resize-none placeholder:text-gray-400 dark:placeholder:text-slate-500"
                  />
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-xs text-gray-400 dark:text-slate-500">
                    ...
                    Se notificará a Administradores, Autorizadores y Capturistas
                  </div>
                  <button
                    onClick={handleEnviarNotificacion}
                    disabled={enviandoNotificacion || !mensajeNotificacion.trim()}
                    className="flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-white bg-gem-primary hover:bg-gem-primary-dark transition-all shadow-lg shadow-gem-primary/30 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    ...
                    {enviandoNotificacion ? 'Enviando...' : 'Enviar a Todos'}
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden animate-in slide-in-from-bottom-4">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="block text-sm font-semibold text-gray-700 dark:text-slate-300">Nombre Completo *</label>
                <input
                  type="text"
                  value={formUser.nombreCompleto}
                  onChange={e => setFormUser({ ...formUser, nombreCompleto: e.target.value })}
                  required
                  className="w-full border border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-900 text-sm dark:text-slate-100 rounded-xl p-3 focus:ring-2 focus:ring-gem-primary/20 focus:border-gem-primary transition-colors"
                />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="block text-sm font-semibold text-gray-700 dark:text-slate-300">Usuario *</label>
                  <input
                    type="text"
                    ...
                    required
                    className="w-full border border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-900 text-sm dark:text-slate-100 rounded-xl p-3 focus:ring-2 focus:ring-gem-primary/20 focus:border-gem-primary transition-colors disabled:bg-gray-100 dark:disabled:bg-slate-800 disabled:text-gray-500 dark:disabled:text-slate-500"
                  />
                </div>
                <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="block text-sm font-semibold text-gray-700 dark:text-slate-300">Rol *</label>
                  <select
                    value={formUser.rol}
                ...
                <div className="space-y-1.5">
                  <label className="block text-sm font-semibold text-gray-700 dark:text-slate-300">Dependencia</label>
                  <input
                    type="text"
            ...
            {/* Footer */}
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-100 dark:border-slate-700 bg-gray-50/50 dark:bg-slate-900/50">
              <button
                onClick={() => setShowModal(false)}
                className="px-4 py-2.5 text-sm font-semibold text-gray-600 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-xl transition-colors"
              >
                Cancelar
              </button>
              <button
